...
//static CGContextRef CreateRGBABitmapContext (CGImageRef inImage)//                                                                                                                                                                                                                     上下文
//{
//	CGContextRef context = NULL;
//	CGColorSpaceRef colorSpace; 
...
- (unsigned char *)RequestImagePixelData:(UIImage *)inImage{
    ...
    CGContextRef cgctx = [self CreateRGBABitmapContext:img]; //使用上面的函数创建上下文
...
- (UIImage*)imageWithImage:(UIImage*)inImage withColorMatrix:(const float*) f andIndex:(int)index
{
    
    UIImage *sourceImage = [self saveImageWithIndex:index andImage:inImage];
    
    NSLog(@"%@",NSStringFromCGSize(inImage.size));
    unsigned char *imgPixel = [self RequestImagePixelData:sourceImage];
	CGImageRef inImageRef = [sourceImage CGImage];
    ...
    NSLog(@"%@",NSStringFromCGSize(myImage.size));
	CFRelease(imageRef);
	CGColorSpaceRelease(colorSpaceRef);
	CGDataProviderRelease(provider);
    imgPixel = NULL;
    UIImage *newImage = [self saveImageWithIndex:index andImage:myImage];
	return newImage;
}



- (UIImage *)saveImageWithIndex:(int )index andImage:(UIImage*)myImage {
    
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSString *fileName = [NSString stringWithFormat:@"ImageUtilCache%i.jpg", index];
